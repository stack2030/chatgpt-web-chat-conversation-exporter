...
ChatGPT Web Chat Conversation Exporter is designed as a local-first browser-console utility.
...
- no Google OAuth, Notion OAuth, Gmail integration, or cloud export flow

The script runs locally in the browser tab where the user has already opened ChatGPT.
...
2. Interacts with the currently open ChatGPT web conversation.
3. Uses same-origin ChatGPT conversation data when available.
4. Falls back to extracting visible conversation content from the page DOM when needed.
5. Stores captured content in browser memory during the run.
6. Downloads a local Markdown file.

## Same-origin note

The script may call ChatGPT same-origin endpoints from the already-open ChatGPT page/session when that extraction path is available.

That means:

```text
browser -> chatgpt.com / OpenAI-owned ChatGPT endpoint
```

It does **not** send conversation content to a third-party server.

If a stricter visible-page-only mode is needed, the script configuration can disable same-origin API extraction and use DOM fallback only.
...
Only use this tool for ChatGPT conversations you are authorized to access and export.
...
If the ChatGPT web interface changes and the exporter breaks, open an issue using the ChatGPT UI breakage template.
